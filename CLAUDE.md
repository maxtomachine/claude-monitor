...
reads every Ghostty window across every Space: tabs in order and the
selected tab from Ghostty's OWN scripting dictionary, frames from
CoreGraphics' window list (`CGWindowListCopyWindowInfo`, all windows,
owner Ghostty, layer 0), joined by tab title since the tabs of one window
are separate NSWindows sharing a frame. The first cut read everything via
System Events AX, which lists only the CURRENT Space's windows: on
2026-08-24, with stakes (Max about to quit Ghostty), it saved 4 of 41
windows and reported success, twice. Verified after the rewrite: 41/41
framed, exact agreement with AX for the 10 AX could see. Each tab's
`·sid8` marker resolves to a full sid against `parse_sessions()`; a tab
with no marker (Claude Nest voice mode titles tabs "◐ name" with none)
resolves by exact title when that names one conversation. Every save
keeps the previous snapshot as `monitor-layout.json.bak` and reports the
window-count delta, so a shrunken save is visible, never silent. It pins every Claude it found, unioned with the
...
**A hotkey ships with its hotkey line and its docs row, mechanically.** The
footer (`DosFooter`) is generated by `footer_items_from_bindings()` from
`ClaudeMonitor.BINDINGS`: a binding's `show` flag is the one switch, so a
new or changed hotkey cannot ship without its footer entry (Max,
2026-08-24: "hotkeys need these so they ship when we add or change one").
`TestHotkeyLineShipsWithEveryHotkey` asserts the footer equals the visible
bindings and that every visible hotkey has a row in the README key table.
Before this the hand-kept footer showed ^H for History (bound to Ctrl+Z)
and omitted Ctrl+L, and the README had no row for `n` or `Ctrl+g`. Add a
hotkey: add the Binding (with `show=True` if it belongs on the line), add
the README row and the row in this table, run the suite.
